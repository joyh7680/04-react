...
function App() {
  return (
    // 7) BrowserRouter : 라우팅이 필요한 컴포넌트들을 감싸는 컴포넌트
    // <BrowserRouter>
     <>
        {/* 1 기본 화면 표시  */}
        <nav>
          {/* 다른 페이지로 이동하는 링크들 */}
          {/* a태그는 서버사이드 방식 => 매번 응답된 새 html이 다시 렌더링(깜빡임) */}
          {/* <a href="/">홈</a> | <a href="/about">소개</a> | <a href="/contact">문의</a> */}
          {/* 8) link 컴포넌트는 클라이언트 사이드 방식 -> import확인 */}
          <Link to="/">홈</Link> | <Link to="/about">소개</Link> | <Link to="/contact">문의</Link>
        </nav>
        <main>
          {/* 특정 url 요청에 있어서 각 페이지마다 보여져야할 컴포넌트 자리  */}
          {/* 4)  Route   : url 요청 주소와 컴포넌트를 매핑해 주는 단위(컴포넌트) */}

          {/* 5) Route 경로 설정 -> import 자동 확인 */}
          {/* 6)  Routes  : Route들을 묶어주는 단위(컴포넌트) */}
          <Routes>
            {/* 9) 경로 바꿔주기 <Route path='/' element={<Home />} /> */}
...
/* 2 렌더링 화면표시 어떻게할건지 생각 -> 3 해당 파일 생성 
[nav] 다른 페이지로 이동하는 링크들
[main 부분]
url 
/         ⇒ Home 컴포넌트
/about   => About 컴포넌트
